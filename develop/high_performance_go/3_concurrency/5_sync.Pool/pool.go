...
/*
Pool是可伸缩（高负载时会动态扩容，存在池中的对象不活跃会被自动清理）、并发安全的临时对象池，大小受限于内存大小。
它用来存放已经分配但暂时不用的临时对象，通过对象重用机制，减少内存分配，缓解GC压力，提高程序性能。

通俗来讲，它本身是一个池子，你要和他建立一个约定，之后你需要什么，就可以从中获取，用完了还可以还回去。但是下次拿的时候，
就不一定是你上次存的那个了。
所以这个池子设计的目的就很明确了，就是为了复用已经使用过的对象，从而优化内存使用和回收。

一开始这个池子会初始化一些对象供你使用，如果不够了，就会通过new产生一些，当你放回去了之后，这些对象会被别人复用，当对象
