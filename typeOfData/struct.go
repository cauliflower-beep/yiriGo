...
/*
结构体内存布局
1.结构体是占用一块连续的内存，一个结构体变量的大小是由结构体中的字段决定的；
	如 Foo , 大小可以通过 unsafe.Sizeof() 函数来计算.
2.但是结构体的大小又不完全由结构体的字段决定；
	如 Bar , 大小不是三个字段的和13字节，而是24字节.
	原因是Go在编译的时候会按照一定的规则自动进行内存对齐。之所以这么设计是为了减少CPU访问内存的次数，加大CPU访问内存的吞吐量。
	如果不进行内存对齐的话，很可能会增加CPU访问内存的次数。
...
type Bar struct {
	x int32 //4
	y *Foo  // 8 32位机器上，指针变量占用4个字节，64位机器上占用8个字节
	z bool  //1
}
