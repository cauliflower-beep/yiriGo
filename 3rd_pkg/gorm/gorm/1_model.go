...
/*
	gorm 允许使用自定义类型，但必须实现两个接口：
		Scan(value interface{}) error
		Value() (driver.Value, error)

	这样 gorm 才能正常识别转化，否则会读不出来：
...
type Character struct {
	/*
		内嵌 gorm 预定义的结构体，具体字段看结构体内容
		其中的 CreatedAt\UpdateAt 是 gorm 自动追踪填充的
	*/
	gorm.Model
	/*
		colunm 是很常用的一个 gorm 标签
		很多时候，数据库的表并不是你创建的，这个时候出现字段匹配不上的现象很正常，
		可以使用 column 来做一个匹配
	*/
	Name     string `gorm:"column:name"`
	Age      uint8  `gorm:"check:age > 12"` // 为 age 字段创建检查约束
	Fight    uint8
	Birthday int64 `gorm:"serializer:unixtime;type:time"` // 跟 int64 类型可以互转
	Pal      string
	/*
		sql自定义的一种类型
	...
	*/
	...
	Hobby    Hobby `gorm:"serializer:json"` // 指定字段如何在数据库中进行序列化和反序列化
	JobInfo1 Job   `gorm:"embedded;embeddedPrefix:job_"`
