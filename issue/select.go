...
/*
死锁问题
如果你执行过 ./block.go 会发现main中的 goroutine 执行完之后，终端报了死锁的错误，原因
就是main中存在 select{}.
select 为什么会存在死锁问题呢？答案都在spec中:
https://go.dev/ref/spec#Select_statements
翻译第一条：在输入“select”语句时，接收操作的通道操作数以及发送语句的通道和右侧表达式仅按源顺序
计算一次。意味着 select 会拿到右侧语句的结果才执行，如果拿不到就会一直堵塞，造成死锁。
*/
/*
可能上述的解释还是不太清楚，来看个例子:
